...
else
{
	...
	$requse++;
	if($req->rowCount() == 0)
		display_error('Recherche', '<p>Aucune réponse n\'a été trouvée pour votre requête</p>
	...
	echo '            <tr>
			  ...
			  if($rang != -1 && $data['id'] != $idmembre) 
				...
				<a href="index.php?page=mpsend&id='.$data['id'].'">
					<img src="'.$design.'actions/fr/mp.gif" alt="Envoyer un message privé" />
				</a>';
		...
		<div class="bottom_infos">
			...
			if($p > 0) 
				echo '<a href="index.php?page=search&amp;p='.($p-1).'&amp;clearsearch">Page précédente</a>';
